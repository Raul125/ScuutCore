...
namespace ScuutCore.Modules.Stalky106
{
	public class Methods
	{
		...
		public Methods(Stalky106 plugin)
		{
			this.plugin = plugin;
		}
		...
		public float StalkyCooldown
		{
			set
			{
				...
				if (plugin.Config.AnnounceReady)
				{
		...
		public IEnumerator<float> AnnounceGlobalCooldown()
		{
			...
			foreach (Player player in Player.List)
			{
				...
				{
					player.Broadcast(6, plugin.Config.StalkReady);
				}
			}
		}
		...
		internal bool Stalk(Player player)
		{
			...
			if (timeDifference > 6f)
			{
				...
				if (cdAux < 0)
				{
					...
					player.Broadcast(6, plugin.Config.DoubleClick);
				}
				return true;
			}
			else
			{
				...
				if (cdAux > 0)
				{
					stalky106LastTime = Time.time;
					int i = 0;
					for (; i < 5 && cdAux > i; i++) player.Broadcast(1, plugin.Config.Cooldown_Message.Replace("$time", (cdAux - i).ToString("00")));
		...
		public IEnumerator<float> StalkCoroutine(Player player)
		{
			...
			foreach (Player plausibleTarget in Player.List)
			{
				...
				if (!alwaysIgnore.Contains(plausibleTarget.Role)
					&& !plugin.Config.IgnoreRoles.Contains(plausibleTarget.Role)
					&& !plugin.Config.IgnoreTeams.Contains(plausibleTarget.Role.Team))
				{
					if (plugin.Config.SameZoneOnly)
					{
			...
			if (list.IsEmpty())
			{
				player.Broadcast(4, plugin.Config.NoTargetsLeft);
				yield break;
			}
			...
			{
				player.Broadcast(4, plugin.Config.Error);
				yield break;
			}
			...
			StalkyCooldown = plugin.Config.Cooldown;
			stalky106LastTime = Time.time;
			disableFor = Time.time + 10f;
			if (!plugin.Config.RoleDisplayNames.TryGetValue(target.Role, out string className))
			{
				className = defaultRoleNames[(int)target.Role.Type];
			}

			;
			player.Broadcast(6, ReplaceAfterToken(plugin.Config.StalkMessage, '$',
									new Tuple<string, object>[] {
										new Tuple<string, object>("player", target.Nickname),
										new Tuple<string, object>("class", className),
										new Tuple<string, object>("cd", plugin.Config.Cooldown)}));
		}
		...
		public IEnumerator<float> PortalProcedure(Scp106Role script, Vector3 pos)
		{
			...
			if (plugin.Config.AutoTp)
			{
				yield return Timing.WaitForSeconds(plugin.Config.AutoDelay);
				...
				&& plugin.Config.ForceAutoTp); // If force_auto_tp, the do-while will only execute once.
			}
		}
