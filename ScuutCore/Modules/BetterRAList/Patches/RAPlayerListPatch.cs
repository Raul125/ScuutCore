...
namespace ScuutCore.Modules.BetterRAList
{
	...
	public class RaPlayerList
	{
		public static bool Prefix(RemoteAdmin.Communication.RaPlayerList __instance, global::CommandSender sender, string data)
		{
			...
			string text = string.IsNullOrEmpty(BetterRAList.Singleton.Config.ResetField) ? string.Empty : $"\n<size=18><color=white>(0) {BetterRAList.Singleton.Config.ResetField}</color></size>\n";

			ReferenceHub[] referenceHubs = global::ReferenceHub.GetAllHubs().Values.ToArray();

			if (BetterRAList.Singleton.Config.SortListByPlayerId)
			...
			foreach (global::ReferenceHub referenceHub in referenceHubs)
			{
				...
				try
				{
					if (string.IsNullOrEmpty(serverRoles.HiddenBadge) || (serverRoles.GlobalHidden && canViewHidenGlobalBadges) || (!serverRoles.GlobalHidden && canViewHidenBadges))
					{
						...
						{
							string groupColor = string.IsNullOrEmpty(BetterRAList.Singleton.Config.PermissionGroupFormat) ? string.Empty : Color.white.ToHex();
							string groupName = string.IsNullOrEmpty(BetterRAList.Singleton.Config.PermissionGroupFormat) || string.IsNullOrEmpty(serverRoles?.Group?.BadgeText) ? string.Empty : (serverRoles.Group.BadgeText.Length < 6) ? serverRoles.Group.BadgeText : serverRoles.Group.BadgeText.Substring(0, 6).Trim();

							if (!string.IsNullOrEmpty(BetterRAList.Singleton.Config.PermissionGroupFormat) && !string.IsNullOrEmpty(serverRoles.Group.BadgeColor))
							{
							...
							raPermissionText = string.IsNullOrEmpty(BetterRAList.Singleton.Config.PermissionGroupFormat) ? "<link=RA_Admin><color=white>[]</color></link> " : string.Format(BetterRAList.Singleton.Config.PermissionGroupFormat, groupColor, groupName);
						}
						else
						{
				...
				text = string.Concat(new object[]
				{
					text,
					raPermissionText,
					isOverwatchEnabled ? (string.IsNullOrEmpty(BetterRAList.Singleton.Config.OverwatchFormat) ? "<link=RA_OverwatchEnabled><color=white>[</color><color=#03f8fc></color><color=white>]</color></link> " : BetterRAList.Singleton.Config.OverwatchFormat) : string.Empty,
					"<color={RA_ClassColor}>(",
